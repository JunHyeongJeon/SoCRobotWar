...
void CManager::SendMsgCam(M_STATE step, char cnt)
{
	...
	printf("[Manager] SEND : CAM\n");
	printf("[Manager] Mission Num = %s\n", m_pCommProc->GetMStateString(step));
...
void CManager::SendMsgRobot(R_STATE motion, MV_STEP motion_cnt)
{
	...
	printf("[Manager] SEND : ROBOT\n");
	printf("[Manager] Motion Num = %d\n", msg[1]);
	printf("[Manager] Motion Count = %d\n\n",  msg[2]);
}
...
void CManager::ReceiveMsg(COMM_TYPE commType)
{
	...
	// CAM 결과 수신
	case PP_CAM_RESULT:
		printf("[Manager] RECV : CAM_RESULT\n");
		...
		printf("[Manager] m_cCamTransRobotMotion = %d\n", m_cRecvMsg[1]);
		printf("[Manager] m_cCamTransRobotMotionCnt = %d\n", m_cRecvMsg[2]);
		printf("[Manager] m_cCamTransStepCmd = %d\n\n", m_cRecvMsg[3]);
		break;
	// 모션 변경 확인 작업
	case PP_ROBOT_STATE_CHANGE_ACK:
		printf("[Manager] RECV : ROBOT_STATE_CHANGE_ACK\n\n");
		break;
	// 모션 끝
	case PP_ROBOT_STATE_END:
		printf("[Manager] RECV : ROBOT_STATE_END\n\n");
		break;
	default:
		printf("[Manager] RECV : DEFAULT(ERROR)\n\n");
		break;
	}
}
